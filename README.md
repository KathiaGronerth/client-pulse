...
[Live Demo](https://client-pulse-ochre.vercel.app/)
...
| **Transparent Health Score** | Score computed at runtime by a deterministic engine (`lib/scoring.js`) with a visible penalty breakdown | Every number traces to a specific Redtail or Nitrogen field. No black box. |
| **AI Call-Prep Brief** | Brief generated by the Anthropic Messages API server-side (`app/api/brief`), with a deterministic rule-based fallback | Advisor gets risk status, last conversation context, and open commitments in 3–5 sentences — with or without an API key |
...
**Nitrogen fields used (in the current build):** Client Risk Number (tolerance), Portfolio Risk Number (actual), risk gap (derived), risk status, 6-month downside, risk trend, Probability of Success. The fixtures also carry GPA and the upside range for shape fidelity; the dashboard does not render them yet.

---

## Why Synthetic Data

Real advisor data is regulated PII — covered by **GLBA** and **SEC/FINRA** privacy and recordkeeping rules — so it can't live in a public demo repo. V1 therefore uses **field-faithful synthetic records** mapped to the actual Redtail and Nitrogen screens and documented API shapes (see `lib/data/fixtures/`). Because the app reads through a data-source adapter layer, moving from synthetic to live client data is a **configuration change, not a redesign**: implement the `RedtailAdapter` / `NitrogenAdapter` stubs and select them in `lib/data/index.js`.
...
| Concern | How | Where |
|---------|-----|-------|
| **Scoring** | Deterministic weighted formula (not ML), regression-tested against fixtures | `lib/scoring.js` |
| **Language generation** | LLM (Anthropic Claude), called server-side, with a deterministic fallback | `app/api/brief/route.ts` |
| **Data** | Adapter pattern — a `DataSource` interface, default `MockAdapter`, stubbed live adapters | `lib/data/` |

**Why split this way:** scores must be **traceable** — an advisor has to be able to explain every number to a client, so scoring is a transparent formula, not a model. The LLM is reserved for the one place language genuinely adds value: turning structured fields into a readable brief — and even there a rule-based brief runs when no key is present. Swapping the mock for a live data source is a one-line change in `lib/data/index.js`; the Redtail/Nitrogen adapters are integration stubs today, and a production wiring would also move data loading to async fetches.
...
2. **Call-Prep Brief** — the Anthropic Messages API (Claude, called server-side) turns structured Redtail + Nitrogen fields into a 3–5 sentence briefing, grounded only in the data provided. The advisor taps "Prepare for Call" and sees a clean summary; a deterministic rule-based brief is used when no key is configured or the call fails. No prompt interface. No chatbot.
...
# then open .env.local and paste your Anthropic API key (from console.anthropic.com)
npm run dev
```
...
| Scoring Engine | Deterministic formula in `lib/scoring.js`, regression-tested (`lib/scoring.test.mjs`) |
| AI Brief Generation | Anthropic Claude API (`claude-opus-4-8`), server-side, with deterministic fallback |
| Data | 8 synthetic client households via a data-source adapter layer (`lib/data`), field-faithful to Redtail + Nitrogen API shapes |
| Deployment | Vercel |
...
client-pulse/
├── app/
│   ├── page.tsx                 # Entry point (mounts ClientPulse)
│   ├── ClientPulse.jsx          # Main prototype component (UI)
│   └── api/brief/route.ts       # Server route: LLM call-prep brief + fallback
├── lib/
│   ├── scoring.js               # Deterministic Health Score engine
│   ├── scoring.test.mjs         # Scoring regression test
│   ├── brief.js                 # Rule-based brief + payload validation
│   └── data/                    # Data-source adapter layer
│       ├── DataSource.js        # Interface (getClients / getRiskData)
│       ├── MockAdapter.js       # Default — reads synthetic fixtures
│       ├── RedtailAdapter.js    # Live-integration stub
│       ├── NitrogenAdapter.js   # Live-integration stub
│       ├── transform.js         # Redtail + Nitrogen → flat view-model
│       ├── data.test.mjs        # Round-trip fixture test
│       └── fixtures/            # Synthetic Redtail / Nitrogen JSON
...
| Carlos & Maria Reyes | 63 | 🟢 Healthy | Task 46 days overdue + minor risk drift |
| William J. Thompson | 75 | 🟢 Healthy | On track — Q1 summary pending |
