...
## AI Call-Prep Brief — Setup

The call-prep brief is generated by the Anthropic Messages API server-side (`app/api/brief/route.ts`). If no API key is configured, the app automatically falls back to a deterministic rule-based brief — so it runs with or without a key.
...
`.env.local` is gitignored and never committed. The key is read only on the server (`process.env.ANTHROPIC_API_KEY`) and never reaches the client bundle.

**Vercel:** add `ANTHROPIC_API_KEY` under **Project → Settings → Environment Variables**, then redeploy. Without it, the deployed app serves the rule-based brief.
